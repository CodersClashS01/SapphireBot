    ...
    public void onCommand(CommandEvent event, Member member, TextChannel channel, String[] args) {
        ...
        if (args.length >= 1 && Tools.isMention(args[0])) {
            ...
            if (event.getMessage().getMentionedMembers().get(0) == member) {
                channel.sendMessage(builder.setTitle(member.getEffectiveName() + " konnte leider niemanden retten...").setImage("https://www2.pic-upload.de/img/35633294/faileed.gif").build()).queue();
            } else {
                channel.sendMessage(builder.setTitle(member.getEffectiveName() + " rettet " + event.getMessage().getMentionedMembers().get(0).getEffectiveName() + " vor dem Zug!").setImage("https://www2.pic-upload.de/img/35633296/safe.gif").build()).queue();
            }
